...
p.add_argument('--index', default=-1, type=int, help='IQUV input index')
...
# Initialize network and load model from saved weights
torch.set_grad_enabled(False)
net = UNet(28, 1, batchnorm=False, dropout=0.3, regression=False, bins=80, bc=64).to(args.device)
saved_network_state = torch.load(f'./models/{args.target}_model.pth')
...
if args.index == -1:
    ...
    train_dataset = HMI_Dataset('./inputs/HMIFull_ZARR/', x_labels=['contin', 'meta', 'iquv'], y_labels=[args.target], bins=80)
    test_dataset = HMI_Dataset('./inputs/HMI2016_ZARR2/', x_labels=['contin', 'meta', 'iquv'], y_labels=[args.target], bins=80)
